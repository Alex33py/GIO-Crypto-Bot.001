...
class MarketDataSimulator:
    ...
    def generate_full_market_data(
        ...
        # Подготовка OHLCV массивов для ADX и ATR
        lookback_adx = 30  # Минимум 14 для ADX, берём 30 для запаса
        ohlcv_data = df.iloc[max(0, current_idx - lookback_adx):current_idx + 1]

        highs = ohlcv_data['high'].tolist()
        lows = ohlcv_data['low'].tolist()
        closes = ohlcv_data['close'].tolist()

        # ✅ НОВОЕ: Вычисляем ATR из OHLCV массивов
