...
class UnifiedScenarioMatcher:
        ...
        try:
            ...
            if best_score < self.observation_threshold:
                ...
                if cvd_value > 2 and ls_ratio > 1.2 and rsi < 50:
                    ...
                    logger.info(
                        f"🟢 FALLBACK LONG {symbol}! CVD={cvd_value:.1f}, LS={ls_ratio:.2f}"  # ✅ ИСПРАВЛЕНО
                    )
                    ...
                    logger.info(
                        f"🔴 FALLBACK SHORT {symbol}! CVD={cvd_value:.1f}, LS={ls_ratio:.2f}"  # ✅ ИСПРАВЛЕНО
                    )

                elif abs(cvd_value) < 2 and 0.9 <= ls_ratio <= 1.1 and volume_ratio > 1.2:  # ✅ ИСПРАВЛЕНО
                    best_match = {
                        "id": "FALLBACK_RANGE",
                        "name": "Consolidation",
                        "direction": "LONG",
    ...
    def _calculate_scenario_score(
        ...
        try:
            ...
            if scenario_id in ['SCN_001', 'SCN_002', 'SCN_003']:
                print(f"🔍 DEBUG _calculate_scenario_score: {scenario_id}, ADX={adx_value:.1f}")
            ...
            if if_block:
                ...
                logger.info(f"⚠️ БЕЗ if блока - используем базовый score для {scenario_id}")
                # Возвращаем БАЗОВЫЙ score вместо 0.20
                base_score = 0.6  # ← 60% для сценариев без if блока
                return base_score
