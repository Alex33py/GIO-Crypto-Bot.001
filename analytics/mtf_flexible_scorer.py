...
class FlexibleMTFScorer:
    ...
    def calculate_alignment(self, trends: dict, required_direction: str) -> dict:
        """
        Вычисление согласованности трендов - ИСПРАВЛЕННАЯ ВЕРСИЯ

        Args:
            trends: {"4h": "bearish", "1h": "bearish", "15m": "neutral"}
            required_direction: "LONG" или "SHORT"
        ...
        """
        # ✅ НОРМАЛИЗАЦИЯ: lowercase + map LONG/SHORT → bullish/bearish
        direction_map = {
            "LONG": "bullish",
            "SHORT": "bearish",
            "bullish": "bullish",
        ...
        required = direction_map.get(required_direction, required_direction).lower()

        # ✅ Если нет данных → разрешить (нет вето)
        if not trends:
            return {
                "score": 1.0,
                "strength": "FLEXIBLE",
        ...
        for tf, weight in self.weights.items():
            trend = trends.get(tf, "neutral").lower()

            if trend == required:
                score += weight
                aligned_tfs.append(tf)
            elif trend == "neutral":
                # Нейтральный - частичный score (50% веса)
                score += weight * 0.5
        ...
        # ✅ Определяем силу alignment (БЕЗ "NONE"!)
        if score >= self.thresholds["STRONG"]:
            ...
            strength = "NEUTRAL"  # ← НЕ "NONE"!

        # ✅ Определяем направление (всегда возвращаем что-то разумное)
        if len(aligned_tfs) > len(misaligned_tfs):
            direction = required
        else:
            direction = "neutral"
    ...
    def adjust_confidence(self, base_confidence: float, mtf_result: dict) -> float:
        ...
        multiplier = multipliers.get(mtf_result["strength"], 1.0)
